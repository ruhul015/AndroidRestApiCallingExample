...
public class TaskManager {
    private static String tag = TaskManager.class.getSimpleName();

    private Context mContext = null;
    private final CopyOnWriteArrayList<TaskQueueItem> mTaskQueue = new CopyOnWriteArrayList<TaskQueueItem>();
    ...
    public TaskBase startTask(final TaskType type, final TaskCallback callback, final TaskParameter param, final Executor executor, final String tag) {
        ...
        final boolean canAdd = canAdd(mTaskQueue, item);

        if (!canAdd) {
            // Already running
            return null;
        }
        Log.v("Task Add", "Task Add " + mTaskQueue.size());

        mTaskQueue.add(item);
        item.base = item.type.createIns();
        item.base.initialize(mContext, this, item.type, item.callback);
    ...
    public void cancelAll() {
        final Iterator<TaskQueueItem> ite = mTaskQueue.iterator();
        while (ite.hasNext()) {
    ...
    public void cancelLogic(final String tag) {
        ...
        while (ite.hasNext()) {
            ...
            if (tag.equalsIgnoreCase(item.taskTag)) {
                item.base.cancel();
                Log.v("Task Cancel", "Task Cancel " + mTaskQueue.size());
                break;
            }
        }
    }
    ...
    public void finished(final TaskBase targetBase) {
        ...
        while (ite.hasNext()) {
            ...
            if (targetBase == item.base) {
                Log.v("Task Remove", "Task Remove " + mTaskQueue.size());
                mTaskQueue.remove(item);
                item = null;
                break;
            }
